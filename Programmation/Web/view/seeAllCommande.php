<div class="col-xs-12">
	<form id="formPanier" class="form" action="controller/controllerSimuPaiement.php" data-reload="view/simuPaiement.php" method="post">
			...
			if($nbCmd == 0)
			{
				...
				foreach($tabCommandes as $cmd)
				{
					...
					echo '<div class="col-xs-12">';
						...
						echo '<input type="number" name="qte_' . $prod->getId() . '"  style="height:auto;" id="qte" class="qteSeeAllCmd" min="1" max="' . $prod->getStock() . '" value="' . $cmd->getQuantite() . '" /></p>';
		...
		<fieldset style="display: none;" class="fieldPastCmd">
			<div><label for="rue">Adresse: </label><textarea cols='10' rows='3' name='rue' id="rue" ></textarea></div>
		...
		<fieldset style="display: none;" class="fieldPastCmd">
			<input type="submit" class="btn btn-primary" value="Passer commande" />
		</fieldset>
	</form>
</div>